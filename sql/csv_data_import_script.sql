...
INSERT INTO dim_time(time_id, time_year, time_date, time_of_day)
-- Set time_id to an integer-represented date value
SELECT DISTINCT to_char(s.time_date, 'YYYYMMDD')::integer, s.time_year, s.time_date, s.time_of_day
FROM staging s
ORDER BY time_year, time_date;
...
INSERT INTO fact_pit_stops(race_id, driver_id, stop, lap, ps_time, duration, milliseconds)
...
-- RACE RESULTS
-- SELECT result_id, race_id, race_name, driver_id, forename, status_id, status, constructor_id, const_name, laps, 
-- passing_time, car_number, grid, car_position, position_text, position_order, driver_rank, milliseconds, fastest_lap,
-- fastest_lap_time, fastest_lap_speed, points
-- FROM staging
-- ORDER BY result_id;

-- SELECT * FROM fact_race_results;

INSERT INTO fact_race_results(result_id, race_id, driver_id, status_id, constructor_id, laps, passing_time, car_number, grid,
driver_position, position_text, position_order, driver_rank, milliseconds, fastest_lap, fastest_lap_time, fastest_lap_speed,
points)
SELECT DISTINCT s.result_id, dr.race_id, dd.driver_id, ds.status_id, dc.constructor_id, s.laps, s.passing_time, s.car_number,
s.grid, s.car_position, s.position_text, s.position_order, s.driver_rank, s.milliseconds, s.fastest_lap, s.fastest_lap_time,
s.fastest_lap_speed, s.points
FROM staging s
INNER JOIN dim_races dr ON dr.race_id = s.race_id
INNER JOIN dim_drivers dd ON dd.driver_id = s.driver_id
INNER JOIN dim_status ds ON ds.status_id = s.status_id
INNER JOIN dim_constructors dc ON dc.constructor_id = s.constructor_id
